...
namespace Pancake.Common
{
    ...
    public struct Polynomial
    {
        ...
        /// <summary>Creates a quadratic of the form <c>ax²+bx+c</c></summary>
        ...
        /// <summary>Creates a cubic of the form <c>ax³+bx²+cx+d</c></summary>
        ...
        private static bool FactorAlmost0(float v) => v.Abs() < 0.00001f;

        /// <summary>Given ax³+bx²+cx+d, returns the net polynomial degree, accounting for values very close to 0</summary>
        /// <param name="a">The cubic factor <c>a</c> in <c>ax³+bx²+cx+d</c></param>
        /// <param name="b">The quadratic factor <c>b</c> in <c>ax³+bx²+cx+d</c></param>
        /// <param name="c">The linear factor <c>c</c> in <c>ax³+bx²+cx+d</c></param>
        /// <param name="d">The constant factor <c>d</c> in <c>ax³+bx²+cx+d</c></param>
        [MethodImpl(INLINE)]
        public static PolynomialDegree GetPolynomialDegree(float a, float b, float c, float d) =>
            FactorAlmost0(a) ? GetPolynomialDegree(b, c, d) : PolynomialDegree.Cubic;
        ...
        /// <param name="c">The constant factor <c>c</c> in <c>ax²+bx+c</c></param>
        [MethodImpl(INLINE)]
        public static PolynomialDegree GetPolynomialDegree(float a, float b, float c) => FactorAlmost0(a) ? GetPolynomialDegree(b, c) : PolynomialDegree.Quadratic;

        /// <summary>Given ax+b, returns the net polynomial degree, accounting for values very close to 0</summary>
        /// <param name="a">The linear factor <c>a</c> in <c>ax+b</c></param>
        /// <param name="b">The constant factor <c>b</c> in <c>ax+b</c></param>
        [MethodImpl(INLINE)]
        public static PolynomialDegree GetPolynomialDegree(float a, float b) => FactorAlmost0(a) ? PolynomialDegree.Constant : PolynomialDegree.Linear;
        ...
        {
            switch (GetPolynomialDegree(a, b, c, d))
            {
                case PolynomialDegree.Constant: return default;
                case PolynomialDegree.Linear: return new ResultsMax3<float>(SolveLinearRoot(c, d));
                case PolynomialDegree.Quadratic: return SolveQuadraticRoots(b, c, d);
                case PolynomialDegree.Cubic: return SolveCubicRoots(a, b, c, d);
        ...
        {
            switch (GetPolynomialDegree(a, b, c))
            {
                case PolynomialDegree.Constant: return default; // either no roots or infinite roots if c == 0
                case PolynomialDegree.Linear: return new ResultsMax2<float>(SolveLinearRoot(b, c));
                case PolynomialDegree.Quadratic: return SolveQuadraticRoots(a, b, c);
                default: throw new InvalidEnumArgumentException();
            }
        }
        ...
        {
            if (GetPolynomialDegree(a, b) == PolynomialDegree.Constant) return null;
            return -b / a;
        }
        ...
        public static Polynomial Lerp(Polynomial a, Polynomial b, float t) =>
            new(t.Lerp(a.fCubic, b.fCubic), t.Lerp(a.fQuadratic, b.fQuadratic), t.Lerp(a.fLinear, b.fLinear), t.Lerp(a.fConstant, b.fConstant));
        ...
        [MethodImpl(INLINE)]
        private static float SolveLinearRoot(float a, float b) => -b / a;

        private static ResultsMax2<float> SolveQuadraticRoots(float a, float b, float c)
        {
        ...
        private static ResultsMax3<float> SolveCubicRoots(float a, float b, float c, float d)
        {
        ...
        // t³+pt+q = 0
        private static ResultsMax3<float> SolveDepressedCubicRoots(float p, float q)
        {
            if (FactorAlmost0(p)) // triple root - one solution. solve x³+q = 0 => x = cr(-q)
                return new ResultsMax3<float>(M.Cbrt(-q));
            ...
            if (discriminant < 0.00001)
            {
                ...
                float GetRoot(int k) => pre * Mathf.Cos((1f / 3f) * M.Acos(acosInner.ClampNeg1to1()) - (M.TAU / 3f) * k);
            ...
            {
                // one root
                float coshInner = (1f / 3f) * M.Acosh((-3 * q.Abs() / (2 * p)) * Mathf.Sqrt(-3 / p));
                float r = -2 * M.Sign(q) * Mathf.Sqrt(-p / 3) * M.Cosh(coshInner);
            ...
            {
                // one root
                float sinhInner = (1f / 3f) * M.Asinh(((3 * q) / (2 * p)) * Mathf.Sqrt(3 / p));
                float r = (-2 * Mathf.Sqrt(p / 3)) * M.Sinh(sinhInner);
                return new ResultsMax3<float>(r);
            }
