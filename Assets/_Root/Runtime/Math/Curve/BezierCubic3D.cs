...
namespace Pancake.Common
{
    ...
    [Serializable]
    public struct BezierCubic3D : IParamCurve3Diff<Vector3>
    {
        ...
        [SerializeField] private Vector3 p0, p1, p2, p3; // the points of the curve
        ...
        [NonSerialized] private bool validCoefficients; // inverted isDirty flag (can't default to true in structs)
        [NonSerialized] private Vector3 c3, c2, c1; // cached coefficients for fast evaluation. c0 = p0
        ...
        [MethodImpl(INLINE)]
        public (Vector3 c3, Vector3 c2, Vector3 c1, Vector3 c0) GetCoefficients()
        {
            ReadyCoefficients();
            return (c3, c2, c1, p0);
        }
        ...
        [MethodImpl(INLINE)]
        public Vector3 GetStartPoint() => p0;

        [MethodImpl(INLINE)]
        public Vector3 GetEndPoint() => p3;
        ...
        [MethodImpl(INLINE)]
        public Vector3 GetThirdDerivative(float t = 0)
        {
            ReadyCoefficients();
        ...
        {
            return new BezierCubic3D(Vector3.LerpUnclamped(a.p0, b.p0, t),
        ...
        public static BezierCubic3D Slerp(BezierCubic3D a, BezierCubic3D b, float t)
        {
            ...
            return new BezierCubic3D(p0, p0 + Vector3.SlerpUnclamped(a.p1 - a.p0, b.p1 - b.p0, t), p3 + Vector3.SlerpUnclamped(a.p2 - a.p3, b.p2 - b.p3, t), p3);
        }
        ...
        {
            Vector3 a = new Vector3(P0.x + (P1.x - P0.x) * t, P0.y + (P1.y - P0.y) * t, P0.z + (P1.z - P0.z) * t);
            float bx = P1.x + (P2.x - P1.x) * t;
            float by = P1.y + (P2.y - P1.y) * t;
            float bz = P1.z + (P2.z - P1.z) * t;
            Vector3 c = new Vector3(P2.x + (P3.x - P2.x) * t, P2.y + (P3.y - P2.y) * t, P2.z + (P3.z - P2.z) * t);
            Vector3 d = new Vector3(a.x + (bx - a.x) * t, a.y + (by - a.y) * t, a.z + (bz - a.z) * t);
            Vector3 e = new Vector3(bx + (c.x - bx) * t, by + (c.y - by) * t, bz + (c.z - bz) * t);
            Vector3 p = new Vector3(d.x + (e.x - d.x) * t, d.y + (e.y - d.y) * t, d.z + (e.z - d.z) * t);
        ...
        public Vector3 ProjectPoint(Vector3 point, int initialSubdivisions = 16, int refinementIterations = 4) =>
            ProjectPoint(point, out _, initialSubdivisions, refinementIterations);

        private struct PointProjectSample
        {
            public float t;
        ...
        private static PointProjectSample[] pointProjectGuesses = {default, default, default};
