    ...
    public class BSpline2D
    {
        ...
        public BSpline2D(Vector2[] points, float[] knots, int degree = 3)
        {
            ...
            if (knots.Length != SplineUtils.BSplineKnotCount(this.points.Length, this.degree))
                throw new ArgumentException($"The knots array has to be of length (degree+pointCount+1). Got an array of {knots.Length} knots, expected ${KnotCount}",
                    nameof(knots));
        }
        ...
        public float GetKnotValueAt(float t) => M.Lerp(knots[degree], knots[knots.Length - degree - 1], t);
        ...
        public Vector2 GetSegmentPoint(int segment, float t)
        {
            ...
            float u = M.Lerp(knotMin, knotMax, t);
        ...
        [NonSerialized] private Vector2[] evalBuffer;
        ...
        public Vector2 Eval(int k, float u)
        {
            ...
            for (int r = 1; r < degree + 1; r++)
            {
                ...
                {
                    float alpha = M.InverseLerpSafe(knots[j + k - degree], knots[j + 1 + k - r], u);
        ...
        private float EvalBasis(int p, int k, float u)
        {
        ...
        public Vector2 GetPointKnotAndWeightByLocalSpan(int point, float t)
        {
            ...
            float u = M.Lerp(knotStart, knotEnd, t);
        ...
        public Vector2 GetPointCollect( float t, ICollection<(Vector2 a, Vector2 b)> lines ) {
            ...
            float tKnot = M.Lerp( knots[degree], knots[knots.Length - degree - 1], t );
