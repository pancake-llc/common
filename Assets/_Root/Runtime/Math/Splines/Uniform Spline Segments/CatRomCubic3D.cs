...
namespace Pancake.Common
{
    ...
    [Serializable]
    public struct CatRomCubic3D : IParamCubicSplineSegment3D
    {
        ...
        {
            return new CatRomCubic3D(Vector3.LerpUnclamped(a.p0, b.p0, t),
        ...
        public BezierCubic3D ToBezier() => new BezierCubic3D(p1, p1 + (p2 - p0) / 6f, p2 + (p1 - p3) / 6f, p2);

        /// <inheritdoc cref="CatRomCubic2D.ToHermite()"/>
        public HermiteCubic3D ToHermite() => new HermiteCubic3D(p1, (p2 - p0) / 2f, p2, (p3 - p1) / 2f);

        /// <inheritdoc cref="CatRomCubic2D.ToBSpline()"/>
        public UBSCubic3D ToBSpline() =>
            new UBSCubic3D((7 * p0 - 4 * p1 + 5 * p2 - 2 * p3) / 6,
