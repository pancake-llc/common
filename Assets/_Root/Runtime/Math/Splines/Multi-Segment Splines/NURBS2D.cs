    ...
    public class NURBS2D
    {
        ...
        public NURBS2D(Vector2[] points, float[] knots, float[] weights, int degree = 3)
        {
            ...
            if (weights != null && weights.Length != points.Length)
                throw new ArgumentException($"The weights array has to match the number of points. Got an array of {weights.Length} weights, expected ${points.Length}",
                    nameof(weights));
        ...
        {
            t = M.Lerp(knots[degree], knots[knots.Length - degree - 1], t);
