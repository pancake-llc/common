...
namespace Pancake.Common
{
    ...
    [Serializable]
    public struct NUCatRomCubic3D : IParamCubicSplineSegment3D
    {
        ...
        void ReadyCoefficients()
        {
            ...
            if (knotCalcMode != KnotCalcMode.Manual)
                (k0, k1, k2, k3) = SplineUtils.CalcCatRomKnots(p0,
                    p1,
                    p2,
                    p3,
                    alpha,
                    knotCalcMode == KnotCalcMode.AutoUnitInterval);
            curve = SplineUtils.CalculateCatRomCurve(p0,
                p1,
                p2,
                p3,
                k0,
        ...
        {
            float a = M.InverseLerp(k0, k1, u);
            float b = M.InverseLerp(k1, k2, u);
            float c = M.InverseLerp(k2, k3, u);
            float d = M.InverseLerp(k0, k2, u);
            float g = M.InverseLerp(k1, k3, u);
            switch (i)
            {
