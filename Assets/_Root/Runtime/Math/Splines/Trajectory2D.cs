    ...
    public struct Trajectory2D
    {
        ...
        public ResultsMax2<float> GetTimesAtHeight(float height)
        {
            ...
            else
            {
                ...
                return new ResultsMax2<float>((mvy + sqrt) / acceleration.y, (mvy - sqrt) / acceleration.y);
            }
        }
        ...
        public static ResultsMax2<float> GetAnglesToReachCoordinateAtSameHeight(float xStart, float speed, float g, float xTarget)
        {
            ...
            if (content.Within(-1, 1))
            {
                ...
                {
                    // 2 solutions
                    return new ResultsMax2<float>(angShallow, angSteep);
                }
            }
            else
            {
        ...
        public static ResultsMax2<float> GetAnglesToReachCoordinate(Vector2 startPosition, float speed, float g, Vector2 target)
        {
            ...
            else if (discriminant > 0)
            {
                ...
                float sqrt = Sqrt(discriminant);
                float gx = g * x;
                return new ResultsMax2<float>(Atan((s2 + sqrt) / gx) + flipOffset, Atan((s2 - sqrt) / gx) + flipOffset);
            }
            else
            {
                // 0 solutions
        ...
        public static float GetHeightPotential(float gravity, float currentHeight, float speed) { return currentHeight + (speed * speed) / (2 * -gravity); }
