...
namespace Pancake.Common
{
    ...
    public static partial class IntersectionTest
    {
        ...
        public static ResultsMax2<Vector2> LinearRectPoints(Vector2 center, Vector2 extents, Vector2 pt, Vector2 dir)
        {
            ...
            if (dir.x.Abs() < FLAT_THRESH)
            {
                if (pt.x.Abs() <= extents.x) // inside - two intersections
                    return new ResultsMax2<Vector2>(new Vector2(center.x + pt.x, center.y - extents.y), new Vector2(center.x + pt.x, center.y + extents.y));
            ...
            if (dir.y.Abs() < FLAT_THRESH)
            {
                if (pt.y.Abs() <= extents.y) // inside - two intersections
                    return new ResultsMax2<Vector2>(new Vector2(center.x - extents.x, center.y + pt.y), new Vector2(center.x + extents.x, center.y + pt.y));
                return default; // outside the box
            }
            ...
            // else, two points
            return new ResultsMax2<Vector2>(new Vector2(center.x + ax, center.y + ay), new Vector2(center.x + bx, center.y + by));
        }
        ...
        [MethodImpl(INLINE)]
        public static bool DiscsOverlap(Vector2 aPos, float aRadius, Vector2 bPos, float bRadius) { return DistanceSquared(aPos, bPos) <= (aRadius + bRadius).Square(); }
    }
}