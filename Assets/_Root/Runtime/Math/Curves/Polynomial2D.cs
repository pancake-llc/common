...
namespace Pancake.Common
{
    ...
    public struct Polynomial2D : IParamCurve3Diff<Vector2>
    {
        ...
        public Vector2 C0 { get => new(x.c0, y.c0); set => (x.c0, y.c0) = (value.x, value.y); }
        public Vector2 C1 { get => new(x.c1, y.c1); set => (x.c1, y.c1) = (value.x, value.y); }
        public Vector2 C2 { get => new(x.c2, y.c2); set => (x.c2, y.c2) = (value.x, value.y); }
        public Vector2 C3 { get => new(x.c3, y.c3); set => (x.c3, y.c3) = (value.x, value.y); }
        ...
        public Vector2 ProjectPoint(Vector2 point, int initialSubdivisions = 16, int refinementIterations = 4) =>
        ...
        public Vector2 ProjectPoint(Vector2 point, out float t, int initialSubdivisions = 16, int refinementIterations = 4)
        {
            ...
            {
                PointProjectSample s = new PointProjectSample {t = tSmp, f = curve.Eval(tSmp), fp = vel.Eval(tSmp)};
        ...
        private ResultsMax3<float> Intersect(Vector2 origin, Vector2 direction, bool rangeLimited = false, float minRayT = float.NaN, float maxRayT = float.NaN)
        {
            ...
            rel.C0 -= origin;
            float y0 = M.Determinant(rel.C0, direction); // transform polynomial into the line space y components
            float y1 = M.Determinant(rel.C1, direction);
            float y2 = M.Determinant(rel.C2, direction);
            float y3 = M.Determinant(rel.C3, direction);
        ...
        public ResultsMax3<float> Intersect(Ray2D ray) =>
        ...
        public ResultsMax3<float> Intersect(LineSegment2D lineSegment) =>
            Intersect(lineSegment.start,
                lineSegment.end - lineSegment.start,
                rangeLimited: true,
                0,
        ...
        /// <param name="ray">The ray to test intersection against</param>
        public ResultsMax3<Vector2> IntersectionPoints(Ray2D ray) =>
            TtoPoints(Intersect(ray.origin,
                ray.dir,
                rangeLimited: true,
                0,
                float.MaxValue));
        ...
        /// <param name="lineSegment">The line segment to test intersection against</param>
        public ResultsMax3<Vector2> IntersectionPoints(LineSegment2D lineSegment) =>
            TtoPoints(Intersect(lineSegment.start,
                lineSegment.end - lineSegment.start,
                rangeLimited: true,
                0,
                lineSegment.LengthSquared));
        ...
        public static Polynomial2D Rotate(Polynomial2D poly, float angle) =>
            new(poly.C0.Rotate(angle), poly.C1.Rotate(angle), poly.C2.Rotate(angle), poly.C3.Rotate(angle));
