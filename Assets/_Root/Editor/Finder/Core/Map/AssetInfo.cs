...
namespace Pancake.Editor.Finder
{
	...
	internal class AssetInfo
	{
		...
		public void UpdateIfNeeded()
		{
			...
			{
				Debug.LogWarning(Finder.LogPrefix + "Can't update Asset since path is not set!");
				return;
			}
			...
			if (!fileInfo.Exists)
			{
				Debug.LogWarning(Finder.LogPrefix + "Can't update asset since file at path is not found:\n" + fileInfo.FullName + "\nAsset Path: " + Path);
				return;
			}
		...
		public int[] GetAllAssetObjects()
		{
			...
			if ((assetType == ReflectionTools.fontType ||
				assetType == ReflectionTools.texture2DType ||
				assetType == ReflectionTools.gameObjectType ||
				assetType == ReflectionTools.defaultAssetType && Path.EndsWith(".dll") ||
				assetTypeName == "AudioMixerController" ||
				Path.EndsWith("LightingData.asset")) &&
				assetType != ReflectionTools.lightingDataAsset
#if UNITY_2020_1_OR_NEWER
			    && assetType != ReflectionTools.lightingSettings
#endif
				)
			{
				...
				for (var i = 0; i < loadedObjects.Length; i++)
				{
					...
					if (assetType == ReflectionTools.gameObjectType)
					{
		...
		private void ActualizePath()
		{
			...
			var actualPath = PathTools.EnforceSlashes(AssetDatabase.GUIDToAssetPath(GUID));
