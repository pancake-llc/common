...
namespace Pancake.Editor.Finder
{
	...
	/// <summary>
	/// Does looks for assets dependencies used at the Finder's Assets Map.
	/// </summary>
	public static class AssetDependenciesSearcher
	{
		...
		/// </summary>
		/// Passed parsers will be used in the Assets Dependencies Map and will affect all Finder references-related functionality.<br/>
		/// Call this before running Finder first time (before Assets Map is created).<br/>
		...
		public static void AddExternalDependenciesParsers(List<IDependenciesParser> parsers)
		{
			...
			{
				Debug.LogError(Finder.ConstructWarning("Empty list passed to AddExternalDependenciesParsers!"));
				return;
			}
		...
		internal static string[] FindDependencies(AssetSettingsKind settingsKind, Type type, AssetKind kind, string path)
		{
			...
			if (settingsKind == AssetSettingsKind.NotSettings)
			{
				...
				var guids = AssetTools.GetAssetsGUIDs(dependencies);
		...
		private static void FillSettingsAssetDependencies(ref HashSet<string> dependenciesGUIDs, string assetPath, AssetSettingsKind settingsKind)
		{
			...
			{
				var scenesInBuildGUIDs = SceneTools.GetScenesInBuildGUIDs(true);
				...
				if (settingsAsset != null && settingsAsset.Length > 0)
				{
					...
					while (sp.Next(true))
					{
						if (sp.propertyType == SerializedPropertyType.ObjectReference)
						{
							...
							if (instanceId != 0)
							{
								var path = PathTools.EnforceSlashes(AssetDatabase.GetAssetPath(instanceId));
