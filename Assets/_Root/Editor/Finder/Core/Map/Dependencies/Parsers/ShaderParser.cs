...
namespace Pancake.Editor.Finder
{
	...
	public class ShaderParser : IDependenciesParser
	{
		public Type Type
		{
			get
			{
				return ReflectionTools.shaderType;
			}
		}
		...
		public List<string> GetDependenciesGUIDs(AssetKind kind, Type type, string path)
		{
			...
			{
				var dependencies = AssetTools.GetAssetImporterDependencies(path);
				if (dependencies != null)
				{
					referencesGUIDs.AddRange(AssetTools.GetAssetsGUIDs(dependencies));
				}
			}
		...
		internal static List<string> ScanFileForIncludes(string filePath)
		{
			...
			foreach (var line in fileLines)
			{
				...
				else if (path.IndexOf('/') != -1)
				{
					...
					var fullPath = PathTools.EnforceSlashes(System.IO.Path.GetFullPath(combinedPath));
				...
				else
				{
					...
					assetPath = PathTools.EnforceSlashes(System.IO.Path.Combine(folder, path));
				}
