	...
	public class AddressableAssetGroupParser : IDependenciesParser
	{
		...
		private static List<string> ExtractReferencedAssets(Object assetGroup)
		{
			...
			if (serializedEntries == null)
			{
				...
				if (serializedEntries == null)
				{
					Debug.LogError(Finder.ConstructError("Can't reach serialize entries in AddressableAssetGroup!"));
					return null;
				}
			}
			...
			{
				Debug.LogError(Finder.ConstructError("Can't find serialize entries array in AddressableAssetGroup!"));
				return null;
			}
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (item == null)
				{
					Debug.LogWarning(Finder.ConstructWarning("Serialize entry from AddressableAssetGroup is null!"));
					continue;
				}
				...
				{
					Debug.LogError(Finder.ConstructError("Can't reach Serialize entry GUID of AddressableAssetGroup!"));
					return null;
				}
