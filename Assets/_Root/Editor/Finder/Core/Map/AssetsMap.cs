namespace Pancake.Editor.Finder
{
    ...
    {
        private const string MapPath = "Library/FinderMap.dat";
        ...
        private static bool UpdateMap(AssetsMap map)
        {
            ...
            {
                Debug.LogError(Finder.LogPrefix + "Assets Map update was canceled by user.");
                return false;
            }
            ...
            foreach (var assetPath in allAssetPaths)
            {
                ...
                var kind = EditorTools.GetAssetKind(assetPath);
                ...
                var rawInfo = new RawAssetInfo {path = PathTools.EnforceSlashes(assetPath), guid = guid, kind = kind,};
            ...
            for (var i = count - 1; i > -1; i--)
            {
...
#endif
                {
                    ...
                    if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 2 of 4",
                            "Checking existing assets in map..." + index + "/" + count,
                            (float) index / count))
                    {
                        EditorUtility.ClearProgressBar();
                        Debug.LogError(Finder.LogPrefix + "Assets Map update was canceled by user.");
            ...
            for (var i = 0; i < count; i++)
            {
...
#endif
                {
                    if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 3 of 4",
                            "Looking for new assets..." + (i + 1) + "/" + count,
                            (float) i / count))
                    {
                        Debug.LogError(Finder.LogPrefix + "Assets Map update was canceled by user.");
                        return false;
                ...
                if (type == null)
                {
                    ...
                    if (loadedAsset == null)
                    {
                        ...
                        {
                            if (!AssetTools.IsAssetScriptableObjectWithMissingScript(rawAssetInfoPath))
                            {
                                Debug.LogWarning(Finder.LogPrefix + "Can't retrieve type of the asset:\n" + rawAssetInfoPath);
                                continue;
                            }
                        }
                        else
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                foreach (var mapAsset in map.assets)
                {
                    ...
                    foreach (var dependency in dependencies)
                    {
                        ...
                        if (mapAsset.Type == asset.Type && asset.Type == ReflectionTools.fontType) continue;

                        var referencedAtInfo = new ReferencedAtAssetInfo() {assetInfo = asset};
                        referencedAtInfos.Add(referencedAtInfo);

                        var referenceInfo = new AssetReferenceInfo() {assetInfo = mapAsset};
        ...
        private static AssetsMap LoadMap(string path)
        {
            ...
            catch (Exception)
            {
                Debug.Log(Finder.LogPrefix +
                          "Couldn't read assets map.\nThis message is harmless unless repeating on every Finder run."); //"\n" + ex);
            }
            finally
            {
                stream.Close();
        ...
        private static AssetSettingsKind GetSettingsKind(string assetPath)
        {
            ...
            if (!string.IsNullOrEmpty(fileName))
            {
                try
                {
                    result = (AssetSettingsKind) Enum.Parse(ReflectionTools.assetSettingsKindType, fileName);
                }
                catch (Exception)
                {
                    // ignored
