...
namespace Pancake.Editor.Finder
{
	...
	internal class TreeModel<T> where T : TreeItem
	{
		...
		{
			if (newData == null)
				throw new ArgumentNullException("newData", Finder.ConstructError("Input data is null. Ensure input is a non-null list!"));
		...
		public void RemoveElements(IList<T> elements)
		{
			foreach (var element in elements)
			{
				if (element == root)
				{
					throw new ArgumentException(Finder.ConstructError("It is not allowed to remove the root element!"));
				}
			}
		...
		{
			if (elements == null)
				throw new ArgumentNullException("elements", Finder.ConstructError("elements is null!"));
			if (elements.Count == 0)
				throw new ArgumentNullException("elements", Finder.ConstructError("elements Count is 0: nothing to add!"));
			if (parent == null)
				throw new ArgumentNullException("parent", Finder.ConstructError("parent is null!"));
		...
		{
			if (newRoot == null)
				throw new ArgumentNullException("newRoot", Finder.ConstructError("newRoot is null!"));

			if (data == null)
				throw new InvalidOperationException(Finder.ConstructError("Internal Error: data list is null!"));

			if (data.Length != 0)
				throw new InvalidOperationException(Finder.ConstructError("AddRoot is only allowed on empty data list!"));
		...
		{
			if (element == null)
				throw new ArgumentNullException("element", Finder.ConstructError("element is null!"));
			if (parent == null)
				throw new ArgumentNullException("parent", Finder.ConstructError("parent is null!"));
		...
		{
			if (insertionIndex < 0)
				throw new ArgumentException(Finder.ConstructError("Invalid input: insertionIndex is -1, client needs to decide what index elements should be reparented at!"));
