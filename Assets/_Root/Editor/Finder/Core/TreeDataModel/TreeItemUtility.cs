...
namespace Pancake.Editor.Finder
{
	...
	internal static class TreeItemUtility
	{
		...
		{
			if (result == null)
				throw new NullReferenceException(Finder.ConstructError("The input 'IList<T> result' list is null!"));
		...
		{
			if (array.Length == 0)
				throw new ArgumentException(Finder.ConstructError("list should have items, count is 0, check before calling ValidateDepthValues!"), "array");

			if (array[0].depth != -1)
				throw new ArgumentException(Finder.ConstructError("list item at index 0 should have a depth of -1 (since this should be the hidden root of the tree). Depth is: " + array[0].depth + "!"), "array");
			...
			for (var i = 0; i < array.Length - 1; i++)
			{
				...
				if (nextDepth > depth && nextDepth - depth > 1)
					throw new ArgumentException(Finder.ConstructError(string.Format("Invalid depth info in input list. Depth cannot increase more than 1 per row. Index {0} has depth {1} while index {2} has depth {3}!", i, depth, i + 1, nextDepth)));
			}

			for (var i = 1; i < array.Length; ++i)
				if (array[i].depth < 0)
					throw new ArgumentException(Finder.ConstructError("Invalid depth value for item at index " + i + ". Only the first item (the root) should have depth below 0!"));

			if (array.Length > 1 && array[1].depth != 0)
				throw new ArgumentException(Finder.ConstructError("Input list item at index 1 is assumed to have a depth of 0!"), "array");
		}
		...
		{
			if (root == null)
				throw new ArgumentNullException("root", Finder.ConstructError("The root is null!"));
