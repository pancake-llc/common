namespace Pancake.Editor.Finder
{
    ...
    internal static class SceneTools
    {
        ...
        public static Scene GetSceneByPath(string path) { return SceneManager.GetSceneByPath(path); }
        ...
        public static OpenSceneResult OpenScene(string path, bool activate = false)
        {
            ...
            {
                Debug.LogError(Finder.ConstructError("Can't open scene since path is absent!"));
                return result;
            }
            ...
            if (!targetScene.isLoaded)
            {
                ...
                {
                    Debug.LogError(Finder.ConstructError("Error while opening scene: " + path + "\n" + e));
                    return result;
                }
                ...
                {
                    Debug.LogError(Finder.ConstructError("Can't open scene since path leads to invalid scene!"));
                    return result;
                }
        ...
        public static bool SaveCurrentModifiedScenesIfUserWantsTo() { return EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo(); }
        ...
        public static void SaveScene(Scene scene) { EditorSceneManager.SaveScene(scene); }
        ...
        public static bool SaveDirtyScenesWithPrompt(Scene[] scenesToSave)
        {
            ...
            var selection = EditorUtility.DisplayDialogComplex("Scene(s) Have Been Modified",
                "Do you want to save the changes you made in the scenes:" + scenesPaths + "\n\nYour changes will be lost if you don't save them.",
                "Save",
                "Don't Save",
                "Cancel");
            if (selection == 0)
        ...
        public static string[] GetScenesInBuildGUIDs(bool includeDisabled = false)
        {
            ...
            return AssetTools.GetAssetsGUIDs(scenesInBuild);
        }
        ...
        public static string[] GetScenesInBuild(bool includeDisabled = false)
        {
            ...
            foreach (var sceneInBuild in scenesForBuild)
            {
                ...
                {
                    scenesInBuild.Add(PathTools.EnforceSlashes(sceneInBuild.path));
                }
            }
        ...
        public static void MarkSceneDirty() { EditorSceneManager.MarkSceneDirty(SceneManager.GetActiveScene()); }
        ...
        public static void ReSaveAllScenes()
        {
            ...
            for (var i = 0; i < allScenesGuids.Length; i++)
            {
                ...
                EditorUtility.DisplayProgressBar("Re-saving scenes...",
                    string.Format("Scene {0} of {1}", i + 1, allScenesGuids.Length),
                    (float) i / allScenesGuids.Length);

                var scenePath = AssetDatabase.GUIDToAssetPath(guid);
                scenePath = PathTools.EnforceSlashes(scenePath);
        ...
        private static bool IsSceneUntitled(Scene scene) { return string.IsNullOrEmpty(scene.name) && string.IsNullOrEmpty(scene.path); }
    }
}