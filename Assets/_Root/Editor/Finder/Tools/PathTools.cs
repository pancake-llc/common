namespace Pancake.Editor.Finder
{
    ...
    internal static class PathTools
    {
        ...
        public static string GetPathRelativeToProject(string path) { return !Path.IsPathRooted(path) ? path : path.Substring(assetsFolderIndex + 1); }

        public static string NicifyAssetPath(string path, bool trimExtension = false) { return NicifyAssetPath(path, AssetKind.Regular, trimExtension); }
        ...
        public static string EnforceSlashes(string path) { return string.IsNullOrEmpty(path) ? path : path.Replace('\\', '/'); }

        // source: UnityEditor.U2D.SpriteAtlasInspector.IsPackable(Object) : bool
        public static string[] GetAllPackableAssetsGUIDsRecursive(string folder) { return AssetDatabase.FindAssets("t:Sprite t:Texture2D", new[] {folder}); }

        public static bool IsAssetsRootPath(string path) { return EnforceSlashes(path) == EnforceSlashes(Application.dataPath); }
    }
}