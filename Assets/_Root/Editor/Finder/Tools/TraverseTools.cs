namespace Pancake.Editor.Finder
{
    ...
    internal class TraverseStats
    {
        ...
        public void Clear() { gameObjectsTraversed = componentsTraversed = propertiesTraversed = 0; }
    }
    ...
    internal static class TraverseTools
    {
        ...
        public static TraverseStats GetStats() { return Stats; }

        public static bool TraverseSceneGameObjects(
            Scene scene,
            bool skipCleanPrefabInstances,
            bool forceAllPrefabsTraverse,
            GameObjectTraverseCallback callback,
            SceneRootTraverseCallback rootTraverseCallback = null)
        {
            var rootObjects = scene.GetRootGameObjects();
            return TraverseRootGameObjects(rootObjects,
                skipCleanPrefabInstances,
                forceAllPrefabsTraverse,
                callback,
                rootTraverseCallback);
        }

        public static bool TraversePrefabGameObjects(
        ...
        public static bool TraverseRootGameObjects(
        ...
        private static bool TraverseGameObjectTree(ObjectTraverseInfo traverseInfo, GameObjectTraverseCallback callback)
        {
            ...
            if (!traverseInfo.inPrefabInstance && !traverseInfo.ForcePrefabsAsRegularObjects)
            {
                ...
                traverseInfo.inPrefabInstance = PrefabTools.IsInstance(traverseInfo.current);
                if (traverseInfo.inPrefabInstance)
                {
                    if (!PrefabTools.IsMissingPrefabInstance(traverseInfo.current))
                    {
                        if (traverseInfo.SkipCleanPrefabInstances)
                        {
                            if (PrefabTools.IsWholeInstanceHasAnyOverrides(traverseInfo.current))
                            {
                                PrefabTools.GetOverridenObjectsFromWholePrefabInstance(traverseInfo.current, out traverseInfo.dirtyComponents);
                            }
                        }
                    }
                    else
        ...
        public static void TraverseGameObjectComponents(ObjectTraverseInfo traverseInfo, ComponentTraverseCallback callback)
        {
            ...
            for (var i = 0; i < components.Length; i++)
            {
                ...
                if (ComponentTools.IsComponentVisibleInInspector(component))
                {
                    visibleIndex++;
                }
