namespace Pancake.Editor.Finder
{
    ...
    internal static class EditorTools
    {
        ...
        internal static SceneTools.OpenSceneResult lastRevealSceneOpenResult;
            ...
            else
            {
                switch (sp.propertyType)
                {
                    case SerializedPropertyType.AnimationCurve:
                        if (sp.animationCurveValue != null)
                        {
                            ...
                            if (sp.animationCurveValue.keys != null)
                            {
                                ...
                                {
                                    stringHash.Append(key.value).Append(key.time).Append(key.outTangent).Append(key.inTangent);
                                }
                            }
                        }
                    ...
                    case SerializedPropertyType.FixedBufferSize:
                        ...
                        break;
                    default:
                        Debug.LogWarning(Finder.LogPrefix + "Unknown SerializedPropertyType: " + sp.propertyType);
                        break;
                }
            }
        ...
        {
            if (ReflectionTools.inspectorWindowType == null)
            {
                Debug.LogError(Finder.ConstructError("Can't find UnityEditor.InspectorWindow type!"));
                return null;
            }

            var inspectorWindow = EditorWindow.GetWindow(ReflectionTools.inspectorWindowType);
            if (inspectorWindow == null)
            {
                Debug.LogError(Finder.ConstructError("Can't get an InspectorWindow!"));
                return null;
            }
        ...
        public static ActiveEditorTracker GetActiveEditorTrackerForSelectedObject()
        {
            ...
            if (ReflectionTools.inspectorWindowType == null) return null;

            inspectorWindow.Repaint();

            ActiveEditorTracker result = null;

            var trackerProperty = ReflectionTools.GetPropertyInfo(ReflectionTools.inspectorWindowType, "tracker");
            ...
            {
                Debug.LogError(Finder.ConstructError("Can't get ActiveEditorTracker from the InspectorWindow!"));
            }
        ...
        public static bool RevealInSettings(AssetSettingsKind settingsKind, string path = null)
        {
            ...
            switch (settingsKind)
            {
                ...
                case AssetSettingsKind.NotSettings:
                    Debug.LogWarning(Finder.ConstructWarning("Can't open settings of kind NotSettings Oo"));
                    result = false;
                    break;
                case AssetSettingsKind.DynamicsManager:
                    if (!MenuTools.ShowProjectSettingsPhysics())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open Physics Settings!"));
                        result = false;
                    }

                    break;
                case AssetSettingsKind.EditorBuildSettings:
                    if (!MenuTools.ShowEditorBuildSettings())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open EditorBuildSettings!"));
                        result = false;
                    }

                    break;
                case AssetSettingsKind.EditorSettings:
                    if (!MenuTools.ShowEditorSettings())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open Editor Settings!"));
                        result = false;
                    }

                    break;
                case AssetSettingsKind.GraphicsSettings:
                    if (!MenuTools.ShowProjectSettingsGraphics())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open GraphicsSettings!"));
                        result = false;
                    }
                ...
                case AssetSettingsKind.Physics2DSettings:
                    if (!MenuTools.ShowProjectSettingsPhysics2D())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open Physics2D Settings!"));
                        result = false;
                    }

                    break;
                case AssetSettingsKind.ProjectSettings:
                    if (!MenuTools.ShowProjectSettingsPlayer())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open Player Settings!"));
                        result = false;
                    }

                    break;
                case AssetSettingsKind.PresetManager:
                    if (!MenuTools.ShowProjectSettingsPresetManager())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open Preset Manager!"));
                        result = false;
                    }
                ...
                case AssetSettingsKind.TagManager:
                    if (!MenuTools.ShowProjectSettingsTagsAndLayers())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open Tags and Layers Settings!"));
                        result = false;
                    }
                ...
                case AssetSettingsKind.VFXManager:
                    if (!MenuTools.ShowProjectSettingsVFX())
                    {
                        Debug.LogError(Finder.ConstructError("Can't open VFX Settings!"));
                        result = false;
                    }
