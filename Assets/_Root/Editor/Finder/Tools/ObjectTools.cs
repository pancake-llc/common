namespace Pancake.Editor.Finder
{
    ...
    internal static class ObjectTools
    {
        public static long GetUniqueObjectId(Object unityObject, bool recursiveCall = false)
        {
            ...
            if (PrefabTools.IsInstance(unityObject))
            {
                var prefabAssetSource = PrefabTools.GetAssetSource(unityObject);
                if (prefabAssetSource != null)
                {
                    ...
                    Debug.LogError(Finder.ConstructError("Couldn't reach asset source: " + unityObject.name), unityObject);
                }
            }
        ...
        private static long GetUniqueObjectIdFromAssetObject(Object unityObject)
        {
            ...
            {
                Debug.LogError(Finder.ConstructError("Can't get path to the asset " + unityObject.name));
            }

            return result;
        }

        public static string GetNicePropertyPath(string fullPropertyPath) { return EditorTools.NicifyName(RemoveArrayStuffFromProperty(fullPropertyPath)); }
        ...
        public static bool IsHiddenInInspector(Object target) { return (target.hideFlags & HideFlags.HideInInspector) != 0; }
        ...
        private static long GetLocalIdentifierInFile(Object unityObject)
        {
            ...
            try
            {
                ReflectionTools.SetInspectorToDebug(serializedObject);
                var serializedProperty = serializedObject.FindProperty("m_LocalIdentfierInFile");
                return serializedProperty.longValue;
            }
            catch (Exception e)
            {
                Debug.LogWarning(Finder.ConstructWarning("Couldn't get data from debug inspector for object " + unityObject.name + " due to this error:\n" + e));
                return -1;
            }
        }
