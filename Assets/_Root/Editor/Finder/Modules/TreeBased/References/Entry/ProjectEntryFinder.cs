...
namespace Pancake.Editor.Finder.Entry
{
	...
	internal static class ProjectEntryFinder
	{
		...
		private static void ProcessProjectScopeTargets(AssetConjunctions assetConjunctions)
		{
			...
			if (type == ReflectionTools.gameObjectType)
			{
				ProcessPrefab(path);
			}
			else if (type == ReflectionTools.sceneAssetType)
			{
				ProcessSceneForProjectLevelReferences(path, assetConjunctions.conjunctions);
			}
			else if (type == ReflectionTools.monoScriptType)
			{
				ProcessScriptAsset(path);
			}
			else if (type != null && (type.BaseType == ReflectionTools.scriptableObjectType ||
			                          type == ReflectionTools.monoBehaviourType))
			{
			...
			foreach (var conjunction in conjunctions)
			{
				...
				if (referencedAtInfo.entries == null || referencedAtInfo.entries.Length == 0)
				{
					...
					if (referencedAtAssetInfo != null &&
					    referencedAtAssetInfo.assetInfo.Type == ReflectionTools.sceneAssetType)
					{
						...
						referencedAtInfo.entries = new[] {newEntry, sceneSpecificEntry};
					         ...
					         referencedAtAssetInfo.assetInfo.Type == ReflectionTools.gameObjectType)
					{
					...
					if (ReferencesFinder.debugMode)
					{
						...
						{
							Debug.LogWarning(Finder.ConstructWarning(
		...
		private static void ProcessPrefab(string path)
		{
			...
			TraverseTools.TraversePrefabGameObjects(prefabRootGameObject, true, false, EntryFinder.OnGameObjectTraverse);
		...
		{
			var openSceneResult = SceneTools.OpenScene(path);
			if (!openSceneResult.success)
			{
				Debug.LogWarning(Finder.ConstructWarning("Can't open scene " + path));
				return;
			}
			...
			TraverseTools.TraverseSceneGameObjects(openSceneResult.scene, true, false, EntryFinder.OnGameObjectTraverse);

			SceneTools.CloseOpenedSceneIfNeeded(openSceneResult);
		}
