...
namespace Pancake.Editor.Finder.Entry
{
	...
	internal static class GenericObjectProcessor
	{
		...
		public static void ProcessObject(Location currentLocation, Object inspectedUnityObject, Object target, EntryAddSettings addSettings, ProcessObjectReferenceHandler processReferenceCallback)
		{
			...
			TraverseTools.TraverseObjectProperties(componentTraverseInfo, (info, sp) =>
			{
				...
				if (sp.propertyType == SerializedPropertyType.ObjectReference && sp.objectReferenceValue != null)
				{
					...
					if (texEnvs != null)
					{
						for (var k = 0; k < texEnvs.arraySize; k++)
						{
							...
							{
								Debug.LogError(Finder.ConstructError("Can't get second.m_Texture from texEnvs at " + inspectedUnityObject.name));
							}
						}
					}
					else
					{
						Debug.LogError(Finder.ConstructError("Can't get m_SavedProperties.m_TexEnvs.Array from material instance at " + inspectedUnityObject.name));
					}
				}
