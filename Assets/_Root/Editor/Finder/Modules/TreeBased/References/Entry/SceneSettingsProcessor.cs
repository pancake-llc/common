...
namespace Pancake.Editor.Finder.Entry
{
	...
	internal static class SceneSettingsProcessor
	{
		...
		private static void CheckNavMesh(TreeConjunction conjunction, int candidateInstanceId)
		{
			...
			{
				Debug.LogError(
					Finder.ConstructError("Can't find m_NavMeshData at the navMeshSettingsObject!"));
			}
		}
		...
		{
			lightmapSettings = lightmapSettings ? lightmapSettings : SettingsTools.GetInSceneLightmapSettings();
		...
		{
			renderSettings = renderSettings ? renderSettings : SettingsTools.GetInSceneRenderSettings();
			...
			{
				Debug.LogError(Finder.ConstructError("Can't find m_HaloTexture at the RenderSettings!"));
			}
			...
			{
				Debug.LogError(Finder.ConstructError("Can't find m_SpotCookie at the RenderSettings!"));
			}
