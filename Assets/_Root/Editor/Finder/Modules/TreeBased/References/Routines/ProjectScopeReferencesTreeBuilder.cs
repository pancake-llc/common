namespace Pancake.Editor.Finder.Routines
{
    ...
    {
        public static ProjectReferenceItem BuildTreeBranch(
            AssetInfo referencedAsset,
            int depth,
            int id,
            List<AssetConjunctions> conjunctionInfoList,
            List<ProjectReferenceItem> results)
        {
            ...
            {
                id = id + results.Count,
                name = PathTools.NicifyAssetPath(referencedAsset.Path, referencedAsset.Kind),
                assetPath = assetPath,
                assetTypeName = assetTypeName,
                assetSize = referencedAsset.Size,
                assetSizeFormatted = EditorTools.FormatBytes(referencedAsset.Size),
                assetIsTexture = assetType != null && (assetType.BaseType == ReflectionTools.textureType),
            ...
            if (referencedAsset.referencedAtInfoList != null && referencedAsset.referencedAtInfoList.Length > 0)
            {
                foreach (var referencedAtInfo in referencedAsset.referencedAtInfoList)
                {
                    ...
                    var childElement = BuildTreeBranch(referencedAtInfo.assetInfo,
                        newDepth,
                        id,
                        conjunctionInfoList,
                        results);
                    ...
                    if (referencedAtType == ReflectionTools.gameObjectType || referencedAtType == ReflectionTools.sceneAssetType ||
                        referencedAtType == ReflectionTools.monoScriptType || referencedAtType == ReflectionTools.monoBehaviourType ||
                        referencedAtType != null && referencedAtType.BaseType == ReflectionTools.scriptableObjectType)
                    {
                        ...
                        else
                        {
                            ...
                            if (tc == null)
                            {
                                tc = new TreeConjunction {referencedAsset = referencedAsset, referencedAtInfo = referencedAtInfo};
        ...
        private static bool AssetIsFilteredOut(AssetInfo referencedAsset, int depth)
        {
            ...
            foreach (var referencedAtInfo in referencedAsset.referencedAtInfoList)
            {
                if (FilterTools.IsValueMatchesAnyFilter(referencedAtInfo.assetInfo.Path, ProjectSettings.References.pathIgnoresFilters)) continue;
                if (referencedAtInfo.assetInfo.Kind == AssetKind.FromPackage) continue;
