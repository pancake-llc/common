...
namespace Pancake.Editor.Finder
{
	...
	internal static class ProjectScopeReferencesFinder
	{
		...
		public static ProjectReferenceItem[] FindAssetsReferences(FilterItem[] assets, bool ignoreClearOption, bool showResults)
		{
			...
			{
				newItem |= FilterTools.TryAddNewItemToFilters(ref lastSearched, asset);
			}
			...
			FinderWindow.ShowAssetReferences();
				...
				if (!searchCanceled)
				{
					...
					else
					{
						if (newTargetAssets != null && newTargetAssets.Length > 0)
						{
							...
							{
								if (result.depth == 0 && FilterTools.IsValueMatchesAnyFilter(result.assetPath, newTargetAssets))
								{
									found = true;
									break;
								}
					...
					Debug.Log(Finder.LogPrefix + ReferencesFinder.ModuleName + " results: " + (resultsCount - 1) +
		...
		public static string[] GetAssetsFromInstances(int[] instanceIDs)
		{
			...
			foreach (var id in instanceIDs)
			{
				...
				path = PathTools.EnforceSlashes(path);
		...
		{
			var canceled = !SceneTools.SaveCurrentModifiedScenes(false);
			...
			if (!canceled)
			{
				...
				for (var i = 0; i < count; i++)
				{
					...
					{
						if (!FilterTools.IsValueMatchesAnyFilter(assetInfo.Path, selectedAssets)) continue;
					}
					    ...
					    ProjectSettings.References.pathIncludesFilters.Length > 0)
					{
						...
						if (!FilterTools.IsValueMatchesAnyFilter(assetInfo.Path, ProjectSettings.References.pathIncludesFilters)) continue;
					}

					// excludes ignored root asset
					if (FilterTools.IsValueMatchesAnyFilter(assetInfo.Path, ProjectSettings.References.pathIgnoresFilters)) continue;
