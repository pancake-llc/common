...
namespace Pancake.Editor.Finder
{
	...
	internal static class HierarchyScopeReferencesFinder
	{
		...
		public static HierarchyReferenceItem[] FindObjectsReferencesInHierarchy(Object[] objects, bool checkGameObjectsComponents, bool showResults = true)
		{
			...
			var allObjects = ObjectTools.GetObjectsFromInstanceIds(lastSearched);
			...
			if (checkGameObjectsComponents)
			{
				for (var i = items.Count - 1; i >= 0; i--)
				{
					...
					foreach (var component in components)
					{
						...
						if (ObjectTools.IsHiddenInInspector(component)) continue;
			...
			{
				FinderWindow.ShowObjectReferences();
		...
		public static HierarchyReferenceItem[] FindHierarchyObjectsReferences(Object[] allObjects, Object[] newObjects, bool showResults = true)
		{
			...
			try
			{
				...
				EditorTools.lastRevealSceneOpenResult = null;
				...
				if (!searchCanceled)
				{
					...
					if (referencesFound == 0)
					{
						...
						FinderWindow.ShowNotification("Nothing found!");
					}
					else
					{
						if (newObjects != null && newObjects.Length > 0)
						{
							...
							else
							{
								...
								FinderWindow.ClearNotification();
							}
						}
						else
						{
					...
					Debug.Log(Finder.LogPrefix + ReferencesFinder.ModuleName + " results: " + referencesFound +
					          " references found in " + sw.Elapsed.TotalSeconds.ToString("0.000", CultureInfo.InvariantCulture) +
					          " seconds.");
				}
				else
				{
					Debug.Log(Finder.LogPrefix + ReferencesFinder.ModuleName + "Search canceled by user!");
				}
			}
			catch (Exception e)
			{
				Debug.LogError(Finder.LogPrefix + ReferencesFinder.ModuleName + ": " + e);
				EditorUtility.ClearProgressBar();
			}
		...
		private static bool LookForObjectsReferencesInHierarchy(Object[] objects, List<HierarchyReferenceItem> results)
		{
			...
			var objectInstanceIds = ObjectTools.GetObjectsInstanceIDs(objects);
			...
			var canceled = HierarchyEntryFinder.FillHierarchyReferenceEntries(objects, (lookAt, lookForInstanceId, settings) =>
			{
				for (var i = 0; i < objectInstanceIds.Length; i++)
				{
					...
					if (instanceId == lookForInstanceId)
					{
						...
						var assetPath = ObjectTools.TryGetObjectAssetPath(lookAt);
		...
		private static ExactReferenceData ObjectToReferencingEntry(Object target)
		{
            ...
            {
	            Debug.LogError(Finder.ConstructError("Couldn't find referenced Game Object from object " + target));
	            return new ExactReferenceData();
            }

            var referencedSettings = new EntryAddSettings
            {
            	componentName = referencedObjectAsComponent != null ? ComponentTools.GetComponentName(referencedObjectAsComponent) : null,
            	componentIndex = referencedObjectAsComponent != null ? ComponentTools.GetComponentIndex(referencedObjectAsComponent) : -1,
            ...
            var assetPath = ObjectTools.TryGetObjectAssetPath(target);
