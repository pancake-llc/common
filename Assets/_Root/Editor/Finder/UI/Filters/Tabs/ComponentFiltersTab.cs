namespace Pancake.Editor.Finder
{
    ...
    {
        internal ComponentFiltersTab(FilterType filterType, FilterItem[] filtersList, SaveFiltersCallback saveCallback)
            : base(filterType, filtersList, saveCallback)
        {
            caption = new GUIContent("Component <color=" + (filterType == FilterType.Includes ? "#02C85F" : "#FF4040FF") + ">" + filterType + "</color>",
                EditorIcons.ScriptIcon);
        }

        protected override void DrawTabHeader()
        {
            EditorGUILayout.LabelField("Here you may specify names of Components you'd like to <color=" + (filterType == FilterType.Includes ? "#02C85F" : "#FF4040FF") +
                                       "><b>" + (filterType == FilterType.Ignores ? "ignore" : "include") + "</b></color> during the Issues Search.\n" +
                                       "You may drag & drop components to this window from the Inspector.",
                UIHelpers.richWordWrapLabel);
            EditorGUILayout.LabelField("Examples:\n" + "MeshFilter\n" + "DOTweenAnimation", EditorStyles.wordWrappedMiniLabel);
        }
        ...
        protected override bool CheckNewItem(ref string newItem)
        {
            ...
            if (!found)
            {
                EditorUtility.DisplayDialog("Can't find specified Component",
                    "Specified component " + newItem + " wasn't found in usual places. Make sure you've entered valid name.",
                    "Cool, thanks!");
            }
            ...
            if (objects != null && objects.Length > 0)
            {
                ...
                for (var i = 0; i < objects.Length; i++)
                {
                    ...
                    if (monoScript != null)
                    {
                        ...
                        if (type.IsSubclassOf(ReflectionTools.componentType))
                        {
                            canDrop = true;
                            break;
                        }
                ...
                if (canDrop)
                {
                    ...
                    if (currentEventType == EventType.DragPerform)
                    {
                        ...
                        for (var i = 0; i < objects.Length; i++)
                        {
                            ...
                            {
                                var added = FilterTools.TryAddNewItemToFilters(ref filters, FilterItem.Create(componentName, FilterKind.Type));
                                needToSave |= added;
