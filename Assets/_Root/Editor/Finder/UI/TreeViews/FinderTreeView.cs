...
namespace Pancake.Editor.Finder
{
	...
	internal abstract class FinderTreeViewItem<T> : TreeViewItem where T : TreeItem
	{
		public readonly T data;

		internal FinderTreeViewItem(int id, int depth, string displayName, T data) : base(id, depth, displayName)
		{
			this.data = data;
			Init();
		}
	...
	internal abstract class FinderTreeView<T> : TreeView where T : TreeItem
	{
		public event Action TreeChanged;

		protected const float ROW_HEIGHT = 25f;
#if UNITY_2019_3_OR_NEWER
		protected const int ICON_WIDTH = 20;
#else
		protected const int IconWidth = 16;
#endif
		protected const int ICON_PADDING = 7;

		protected readonly List<TreeViewItem> rows = new List<TreeViewItem>(100);

		private TreeModel<T> TreeModel { get; set; }
		private string _newSearchString;

		protected FinderTreeView(TreeViewState state, TreeModel<T> model) : base(state)
		{
			Init(model);
		}

		protected FinderTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader, TreeModel<T> model) : base(state, multiColumnHeader)
		{
		...
		{
			_newSearchString = newSearch;
		}

		public FinderTreeViewItem<T> GetRow(int id)
		{
			foreach (var row in rows)
			{
				if (row.id == id)
				{
					return row as FinderTreeViewItem<T>;
				}
			}
		...
		protected override IList<TreeViewItem> BuildRows(TreeViewItem root)
		{
			...
			{
				Debug.LogError(Finder.ConstructError("tree model root is null. did you call SetData()?"));
				return rows;
			}
		...
		{
			var item = (FinderTreeViewItem<T>)args.item;
		...
		private void AddChildrenRecursive(T parent, int depth, IList<TreeViewItem> result)
		{
			foreach (var treeElement in parent.Children)
			{
				...
				if (!string.IsNullOrEmpty(_newSearchString) && !child.CanBeFoundWith(_newSearchString) && depth == 0) continue;
		...
		protected virtual void DrawCell(ref Rect cellRect, FinderTreeViewItem<T> genericItem, int columnValue, RowGUIArgs args)
		{
			baseIndent = genericItem.depth * GetDepthIndentation();
			CenterRectUsingSingleLineHeight(ref cellRect);
		}

		protected void SelectRowInternal(FinderTreeViewItem<T> rowLocal)
		{
