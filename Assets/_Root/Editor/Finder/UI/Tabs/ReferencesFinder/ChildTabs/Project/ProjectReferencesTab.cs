...
namespace Pancake.Editor.Finder
{
	...
	internal class ProjectReferencesTab : ReferencesChildTab
	{
		...
		{
			get { return EditorIcons.ProjectViewIcon; }
		}

		private readonly ProjectReferencesTreePanel _treePanel;

		public ProjectReferencesTab(FinderWindow window) : base(window)
		{
			_treePanel = new ProjectReferencesTreePanel(window);
		}
			...
			using (new GUILayout.HorizontalScope())
			{
				...
				using (new GUILayout.VerticalScope())
				{
					...
					if (UIHelpers.ImageButton("All assets",
						"Traverses whole project to find where all assets are referenced.", Uniform.FindIcon))
					{
					...
					if (UIHelpers.ImageButton("Selected assets",
						"Adds selected Project View assets to the current search results.", Uniform.FindIcon))
					{
		...
		{
			_treePanel.SelectItemWithPath(autoSelectPath);
		}
