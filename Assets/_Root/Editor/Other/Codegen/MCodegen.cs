    ...
    public static class MCodegen
    {
        class SplineType
        {
            ...
            public SplineType(
                ...
                string prettyName,
                string matrixName,
                RationalMatrix4x4 charMatrix,
                string[] paramNames,
                string[] paramDescs,
                string[] paramDescsQuad = null)
            {
                this.degree = degree;
            ...
            public void AppendParamStrings(CodeGenerator gen, int degree, int i) { gen.Param(paramNames[i], paramDescs[i]); }
        }

        #region Type Definitions

        static SplineType typeBezier = new SplineType(3,
            "Bezier",
            "Bézier",
            "cubicBezier",
            ...
            new[]
            {
                "The starting point of the curve", "The second control point of the curve, sometimes called the start tangent point",
                "The third control point of the curve, sometimes called the end tangent point", "The end point of the curve"
            });

        static SplineType typeBezierQuad = new SplineType(2,
            "Bezier",
            "Bézier",
            "quadraticBezier",
            (RationalMatrix4x4) CharMatrix.quadraticBezier,
            new[] {"p0", "p1", "p2"},
            new[] {"The starting point of the curve", "The middle control point of the curve, sometimes called a tangent point", "The end point of the curve"});

        static SplineType typeHermite = new SplineType(3,
            "Hermite",
            "Hermite",
            "cubicHermite",
            ...
            new[]
            {
                "The starting point of the curve", "The rate of change (velocity) at the start of the curve", "The end point of the curve",
                "The rate of change (velocity) at the end of the curve"
            });

        static SplineType typeBspline = new SplineType(3,
            "UBS",
            "B-Spline",
            "cubicUniformBspline",
            ...
            new[]
            {
                "The first point of the B-spline hull", "The second point of the B-spline hull", "The third point of the B-spline hull",
                "The fourth point of the B-spline hull"
            });

        static SplineType typeCatRom = new SplineType(3,
            "CatRom",
            "Catmull-Rom",
            "cubicCatmullRom",
            ...
            new[]
            {
                ...
                "The second control point, and the start of the catmull-rom curve", "The third control point, and the end of the catmull-rom curve",
        ...
        static void GenerateMatrix(int count, int dim)
        {
            ...
            using (code.BracketScope("namespace Pancake.Common"))
            {
            ...
            // save/finalize
            string path = $"Assets/_Root/Runtime/Math/Numerics/{typeName}.cs";
        ...
        static void GenerateType(SplineType type, int dim)
        {
            ...
            using (code.BracketScope("namespace Pancake.Common"))
            {
                ...
                using (code.BracketScope($"[Serializable] public struct {structName} : IParamSplineSegment<{polynomType},{pointMatrixType}>"))
                {
                    ...
                    code.Append(
                        $"public {pointMatrixType} PointMatrix {{[MethodImpl( INLINE )] get => pointMatrix; [MethodImpl( INLINE )] set => _ = ( pointMatrix = value, validCoefficients = false ); }}");
                    for (int i = 0; i < ptCount; i++)
                    {
                        code.Summary(pointDescs[i]);
                        code.Append(
                            $"public {dataType} {points[i].ToUpperInvariant()}{{ [MethodImpl( INLINE )] get => pointMatrix.m{i}; [MethodImpl( INLINE )] set => _ = ( pointMatrix.m{i} = value, validCoefficients = false ); }}");
                    }
                    ...
                    using (code.BracketScope($"public {dataType} this[ int i ]"))
                    {
                        string indexException =
                            $"throw new ArgumentOutOfRangeException( nameof(i), $\"Index has to be in the 0 to {degree} range, and I think {{i}} is outside that range you know\" )";
                        code.Append($"get => i switch {{ {JoinRange(", ", i => $"{i} => {points[i].ToUpperInvariant()}")}, _ => {indexException} }};");
                        code.Append(
                            $"set {{ switch( i ){{ {JoinRange(" ", i => $"case {i}: {points[i].ToUpperInvariant()} = value; break;")} default: {indexException}; }}}}");
                    }
                    ...
                    if (degree == 3)
                    {
                        ...
                        {
                            nameof(BezierCubic1D).Replace("1D", $"{dim}D"), nameof(HermiteCubic1D).Replace("1D", $"{dim}D"),
                            nameof(CatRomCubic1D).Replace("1D", $"{dim}D"), nameof(UBSCubic1D).Replace("1D", $"{dim}D")
                        };
                        RationalMatrix4x4[] typeMatrices = {CharMatrix.cubicBezier, CharMatrix.cubicHermite, CharMatrix.cubicCatmullRom, CharMatrix.cubicUniformBspline};
            ...
            string path = $"Assets/_Root/Runtime/Math/Splines/Uniform Spline Segments/{structName}.cs";
            File.WriteAllLines(path, code.content);
        }
